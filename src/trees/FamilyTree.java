...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			...
			throw new TreeException("invalid input, does not contain \":\"");
		...
		else
		{
			...
			if (parentNode == null) {
				throw new TreeException("error message");
			}
		...
		for (int i = 0; i < childrenArray.length ; ++i) {
			TreeNode newNode = new TreeNode(childrenArray[i]);
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		TreeNode node1 = root.getNodeWithName(node1.name);		// node whose name is name1
		if (node1 == null)
			throw new TreeException("node1 is null");
			//??? Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(node2.name);		// node whose name is name2
		if (node2 == null)
